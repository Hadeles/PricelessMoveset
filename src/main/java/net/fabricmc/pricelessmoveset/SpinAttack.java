...
public class SpinAttack {
    ...
    public void tick() {
        ...
        if (time <= lastSpinAttackUseTime + SPIN_ATTACK_COOLDOWN_TIME)
            return;
    ...
    public static void spinAttack(ServerPlayerEntity player) {
        ...
        for (Entity entity : list) {
            ...
            if (entity.squaredDistanceTo(vec3d) > squaredRadius)
                continue;
            ...
            if (entity instanceof LivingEntity) {
                ...
                if (player.isTeammate(livingEntity))
                    continue;
        ...
        if (f > 0.0f || g > 0.0f) {
            ...
            if (bl6) {
                if (i > 0) { // Apply knockback
                    if (target instanceof LivingEntity) {
                        ((LivingEntity) target).takeKnockback((float) i * 0.5f,
                                MathHelper.sin(player.getYaw() * ((float) Math.PI / 180)),
                                -MathHelper.cos(player.getYaw() * ((float) Math.PI / 180)));
                    } else {
                        target.addVelocity(
                                -MathHelper.sin(player.getYaw() * ((float) Math.PI / 180)) * (float) i * 0.5f, 0.1,
                                MathHelper.cos(player.getYaw() * ((float) Math.PI / 180)) * (float) i * 0.5f);
                    }
                ...
                if (target instanceof LivingEntity) {
                    ...
                    if (player.world instanceof ServerWorld && m > 2.0f) {
                        ...
                        ((ServerWorld) player.world).spawnParticles(ParticleTypes.DAMAGE_INDICATOR, target.getX(),
                                target.getBodyY(0.5), target.getZ(), n, 0.1, 0.0, 0.1, 0.2);
                    }
                }
            }
        }
